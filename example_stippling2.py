...
def main():
    ...
    points = Lloyd.lloyd_relaxation(initial_points, (0, width, 0, height), alpha=1.0, iterations=20)
    ...
    sample.apply_forward_stippling(points, tone_levels=5)
    forward_screen = sample.screen.copy()
    forward_styles = sample.get_styles()
    
    # 4. Apply Reverse Stippling (Algorithm 2)
    # Placing light stipples on black background.
    print("Applying Reverse Multi-tone Color Stippling...")
    sample.apply_reverse_stippling(points, tone_levels=5)
    reverse_screen = sample.screen.copy()
    reverse_styles = sample.get_styles()
    ...
    axes[0].set_ylim(height/2, height)
    ...
    axes[2].set_facecolor('k')
    axes[2].patch.set_visible(True)
